...
## info
`exclude` file lives here, which contains repo-specific ignore rules.
...
A: `.gitignore` is version-controlled and shared with whoever clones the repo. `exclude` is specific to the local clone - use for files like `.vscode/`.